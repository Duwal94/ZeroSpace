...
from cryptography.hazmat.primitives.ciphers import ChaCha20Poly1305
...
def UploadFile():
    ...
    with open(file_path,"rb") as file:
        ...
        crypt = ChaCha20Poly1305.new(file_key)
        key_crypt = ChaCha20Poly1305.new(master_key)
        try:
            _encr = ModifiedChaCha20Poly1305(file_key, master_key)
        except:
            # _encr= ChaCha20Poly1305.new(file_key, master_key)
            throw_error()
        encrypted_file_key = key_crypt.encrypt(file_key, None)
        encrypted_data = crypt.encrypt(PKCS7(128).pad(file.read()))
        h = hmac.HMAC(master_key, hashes.SHA512())
        ...
        resp = requests.put(url + "files/upload",auth=BearerAuth(token),files={"encrypted_file":temp},data={"encrypted_file_key":encrypted_file_key.hex(),"hmac":hmac_digest.hex()})
        temp.close()
        if resp.status_code != 200:
            print(resp)
...
def GetFileById():
    ...
    key_crypt = ChaCha20Poly1305.new(master_key)
    file_key = key_crypt.decrypt(encrypted_file_key, None)
    ...
    with open(file_name,"wb") as file:
        ...
        file_data = unpadder.update(crypt.decrypt(resp.content, None)) + unpadder.finalize()
